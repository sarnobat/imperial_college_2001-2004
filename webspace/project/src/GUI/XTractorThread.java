...
public class XTractorThread implements Runnable {
	...
	public void requestDatabaseCreation(XERModel model, File xsdFile, DialogWriter dialogWriter,String comment) {
		this.model = model;
		...
		while (true) {
			...
			switch (option) {
				...
				case CREATE_DATABASE :
					{
						XTractor.createRelationalDatabase(model, xsdFile, createDBdialogWriter,comment);
						break;
					}
				...
				case EXPORT_FILE :
					{
						File xmlFile = XTractor.exportData(exportDatabaseSchema,selectedDocumentNumber,exportDialogWriter);
						break;
					}
				case NONE :
					{
