...
public class ImportDatabaseManager extends PostDatabaseManager {
	...
	//	Map from table name (String) to collection of column names (List of String)
	protected Map tableNameToColumnNames;
	// Map from tableName.columnName (concatenated String) to SQL Type (Integer)
	protected Map columnNamesToColumnTypes; 

	public ImportDatabaseManager(String databaseSchemaName) {
		super(databaseSchemaName);

		this.tableNameToColumnNames = new HashMap();
		this.columnNamesToColumnTypes = new HashMap();
	...
	private void populateTableColumnNames() {
		try {
			...
			while (rs.next()) {
				...
				ResultSet rs1 = metaData.getColumns(null, schemaName, tableName, "%");
				while (rs1.next()) {
					String columnName = rs1.getString("column_name");
					// Column type is of type java.lang.Short
					Short columnType = (Short) rs1.getObject("data_type");
					columnNames.add(columnName);
					columnNamesToColumnTypes.put(tableName + "." + columnName, new Integer(columnType.intValue()));
				}
				this.tableNameToColumnNames.put(tableName, columnNames);
			}
		}
		catch (SQLException e) {
			logger.error("Couldn't determine column names for tables. " + e);
		}
		logger.debug("Finished getting table column names");
	}
	...
	protected Map getColumnTypesForTable(String tableName) throws SQLException {
		Map columnNamesToTypes = new HashMap();
		for (Iterator iter = columnNamesToColumnTypes.keySet().iterator(); iter.hasNext();) {
			String qualifiedColumnName = (String) iter.next();
			Integer columnSQLType = (Integer) columnNamesToColumnTypes.get(qualifiedColumnName);
			String unqualifiedColumnName = qualifiedColumnName.split("\\.")[1];
			columnNamesToTypes.put(unqualifiedColumnName, columnSQLType);
		}
	...
	public void addData(String originalElementName, Map attributeNamesToValues) throws AddDataException {
		...
		// Resolve into Database Nomenclature		
		String dataTableName = getDataTableName(originalElementName);
		Map allColumnNamesToValues = getAttributeAliases(attributeNamesToValues);
		// Only a subset of these columns will be added to the table, because some are not present

				
		// Populate the map with a subset of the column names to values
		Collection presentColumnNames = (Collection) tableNameToColumnNames.get(getUnqualifiedTableName(dataTableName));
		Map presentColumnNamesToValues = new HashMap();		
		for (Iterator iter = allColumnNamesToValues.keySet().iterator(); iter.hasNext();) {
			String columnName = (String) iter.next();
			if (presentColumnNames.contains(columnName)) {
				presentColumnNamesToValues.put(columnName,allColumnNamesToValues.get(columnName));
			}
		}

		// Get the types for the column
		Map columnNamesToTypes = null;
		columnNamesToTypes = getColumnTypesForTable(dataTableName, new HashSet(presentColumnNames));

		try {

			performInsert(dataTableName, presentColumnNamesToValues, columnNamesToTypes);
			logger.info("Successfully added data. <" + originalElementName + ">");
		}
		catch (SQLException e) {
			logger.debug("Unable to insert data. Attempting update...");
			try {
				performUpdate(dataTableName, presentColumnNamesToValues, columnNamesToTypes);
			}
