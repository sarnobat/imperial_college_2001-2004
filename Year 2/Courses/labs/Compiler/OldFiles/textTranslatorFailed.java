...
{//All methods will have to be static
	...
	public static void translateText(MAST tree,PrintStream file)
	{
		...
		{
			translateStatList((MAST) ast.getFirstChild().getNextSibling());
		}
	}
	...
	public static void translateStatList(MAST ast)
	{
		if (ast == null)
		{
			return;
		...
		if (ast.getType() == Decm2LexerTokenTypes.BEGIN)
		{
			...
			{
				translateStat(firstStat);
				firstStat = (MAST) firstStat.getNextSibling();
			}
		}
	}

	public static void translateStat(MAST ast)
	{
		if (ast != null)
		{
			switch(ast.getType())
	...
	public static void translateAssignStat(MAST ast,int firstFreeReg)////CONTINUE WORKING HERE
	{
	...
	public static void translateRepeat(MAST ast,int firstFreeReg)
	{
		...
		translateCond(condRoot,firstFreeReg,bodyLabel,endLabel,notInverted);
		codeout.println(endLabel+":");//THIS LINE MAY NOT BE NEEDED
	}
