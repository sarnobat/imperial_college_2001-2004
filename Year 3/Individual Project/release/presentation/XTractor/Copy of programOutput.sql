...
INSERT INTO XTractor_schemas(name,comment) VALUES ('ss','The parts and suppliers schema');
CREATE SCHEMA ss;
...
--------------------	transitor	--------------------
CREATE TABLE ss.transitor(
	color	VARCHAR(50),
	pno	INT,
	rrp	FLOAT,
	PRIMARY KEY (pno)
);INSERT INTO ss.CATALOG_attributes(attribute,element,alias) VALUES ('colour','transitor',NULL);INSERT INTO ss.CATALOG_attributes(attribute,element,alias) VALUES ('pno','transitor',NULL);INSERT INTO ss.CATALOG_attributes(attribute,element,alias) VALUES ('price','transitor',NULL);
CREATE TABLE ss.META_transitor(
	id	SERIAL	PRIMARY KEY,
	pid	INT	NOT NULL	REFERENCES ss.META_ps_db(id),
	ord	INT	NOT NULL,
	pno	INT	REFERENCES ss.transitor(pno)
);
...
CREATE TABLE ss.ps_db_choice (
	...
	pno	INT,
	FOREIGN KEY (pno) REFERENCES ss.transitor(pno),
...
INSERT INTO ss.CATALOG_generalizations(general_table,specialization_table) VALUES ('ps_db_choice','transitor');
...
ALTER TABLE ss.supplies ADD CONSTRAINT supplies_fkey FOREIGN KEY (supplies) REFERENCES ss.transitor(pno);


UPDATE ss.CATALOG_attributes SET alias='rrp' WHERE attribute='price';UPDATE ss.CATALOG_attributes SET alias='color' WHERE attribute='colour';UPDATE ss.CATALOG_elements SET alias='transitor' WHERE element='part';
--------------------
------ VIEWS -------
--------------------
...
CREATE VIEW ss.COMPLETE_transitor AS (
	SELECT ss.meta_transitor.pid,ss.meta_transitor.id,ss.transitor.color,ss.meta_transitor.pno,ss.meta_transitor.ord,ss.transitor.rrp
	FROM ss.meta_transitor
	INNER JOIN ss.transitor
	ON ss.transitor.pno=ss.meta_transitor.pno
);
...
CREATE VIEW ss.CATALOG_ids_to_tables AS (
	...
	(SELECT id,text 'transitor' AS table_name FROM ss.meta_transitor) 
);