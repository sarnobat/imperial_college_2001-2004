...
public class PostDatabaseManager extends AbstractDatabaseManager {
	...
	private void populateAttributeNames() {
		String sql =
			"SELECT * FROM " + getSystemTableName(Nomenclature.ATTRIBUTES);
		ResultSet rs = executeQuery(sql);
		try {
			while (rs.next()) {
				String originalName =
					rs.getString(Nomenclature.ATTRIBUTES_name);
				String databaseEquivalent =
					rs.getString(Nomenclature.ATTRIBUTES_alias);
	...
	private void populateElementNames() {
		String sql =
			"SELECT * FROM " + getSystemTableName(Nomenclature.ELEMENTS);
		ResultSet rs = executeQuery(sql);
		try {
			while (rs.next()) {
				String originalName = rs.getString(Nomenclature.ELEMENTS_name);
				String databaseEquivalent =
					rs.getString(Nomenclature.ELEMENTS_alias);
	...
	public boolean isAutoKey(String keyName) {
		...
		try {
			...
			logger.error(
				"Couldn't determine if key relies on a sequence. " + e);
		}
		return false;
	}
	...
	public boolean isMultiValued(String originalElementName) {
		...
		try {
			ResultSet rs =
				metaData.getColumns(null, schemaName, elementName, elementName);
			if (rs.next()) {
				return true;
			}
			...
			logger.error(
				"Couldn't deterine whether "
					+ elementName
					+ " is a multi-occurring element. "
					+ e);
		}
		return false;
	}
	...
	public List getPrimaryKeyAttributesNames(String originalElementName) {
		...
		try {
			ResultSet rs =
				metaData.getPrimaryKeys("pg_class", schemaName, elementName);
			while (rs.next()) {
	 ...
	 */
	protected Map getColumnTypesForTable(String tableName)
		throws SQLException {
	...
	public Map getAttributeValues(
		...
		try {
			...
			logger.error(
				"Couldn't get attribure values for element "
					+ elementName
					+ ". "
					+ e);
	...
	/**
			...
			*/
	public Map getPrimaryKeyNamesAndValues(
		String originalElementName,
		int id) {
		String elementName = getElementAlias(originalElementName);
		...
		List primaryKeyNames =
			getPrimaryKeyAttributesNames(originalElementName);
		...
		try {
			...
			logger.error(
				"Couldn't determine key values for " + elementName + ". " + e);
		...
		try {
			...
			logger.error(
				"Couldn't determine if "
					+ elementName
					+ " was part of a generalization hierarchy. "
					+ e);
			return false;
		}
	}
	...
	protected Map getAttributeAliases(Map attributeNamesToValues) {
		...
		for (Iterator iter = attributeNamesToValues.keySet().iterator();
			...
			columnNamesToValues.put(
				attributeAlias,
				attributeNamesToValues.get(originalAttributeName));
		}
