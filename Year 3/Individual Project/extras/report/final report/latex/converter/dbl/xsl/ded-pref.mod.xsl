<?xml version='1.0'?>
<!--############################################################################# 
|	$Id: ded-pref.mod.xsl,v 1.3 2003/03/26 21:41:08 rcasellas Exp $
|- #############################################################################
|	$Author: rcasellas $
|														
|   PURPOSE:
...
<xsl:stylesheet 
	...
	<xsl:if test="$subtitle != ''">
	    <xsl:text>{\sc </xsl:text><xsl:copy-of select="$subtitle"/><xsl:text>}&#10;</xsl:text>
	</xsl:if>
    </xsl:template>
	...
	<xsl:call-template name="label.id"/>
	<xsl:text>&#10;\chapter*{</xsl:text><xsl:apply-templates select="." mode="title.ref"/><xsl:text>}&#10;</xsl:text>
    </xsl:template>

    <xsl:template name="preface.subtitle">
	<xsl:variable name="subtitle"><xsl:apply-templates select="." mode="subtitle.content"/></xsl:variable>
	<xsl:if test="$subtitle != ''">
	    <xsl:text>{\sc </xsl:text><xsl:copy-of select="normalize-space($subtitle)"/><xsl:text>}&#10;</xsl:text>
	</xsl:if>
    </xsl:template>
    ...
    <xsl:template match="preface/simplesect">
