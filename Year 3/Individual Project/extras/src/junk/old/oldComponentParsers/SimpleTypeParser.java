 ...
 *
 */
public class SimpleTypeParser extends AbstractParser{

	Logger logger = Logger.getLogger(this.getClass());

	public SimpleTypeParser(XSDSchemaManipulator schemaWalker, XERBuilder xerBuilder) {
	 ...
	 * @return
	 */
	public XERAttribute parseSimpleType(Element xsdSimpleTypeElement, XEREntity parentEntity, String xerAttributeName, String minOccurs) {
		Collection children = xsdSimpleTypeElement.getChildren();
		for (Iterator iter = children.iterator(); iter.hasNext();) {
			...
			if (childComponent.getName().equals("list")) {
				...
				return parseRestriction(childComponent, parentEntity, xerAttributeName, minOccurs);
			}
			else {
	 ...
	 */
	public XERAttribute parseRestriction(Element xsdRestrictionElement, XEREntity parent, String xerAttributeName, String minOccurs) {
		//I think this is compulsory, so no need to deal with null
		String baseType = xsdRestrictionElement.getAttributeValue("base");
		XERAttribute attribute = xerBuilder.getXERFactory().createXERAttributeFromRestriction(xerAttributeName, baseType, minOccurs, parent);
