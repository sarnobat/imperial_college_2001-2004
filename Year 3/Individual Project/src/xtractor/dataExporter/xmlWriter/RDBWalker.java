...
public class RDBWalker {
	...
	public Document writeXMLFile(int rootId, Writer fileWriter) throws IOException {
		...
		String rootElementName = databaseManager.getOriginalElementName(rootElementAlias);
		Element root = new Element(databaseManager.getOriginalElementName(rootElementName));
	...
	private void addChildElements(int parentId, Element parent) throws IOException {
		...
		for (Iterator iter = ordsToIds.keySet().iterator(); iter.hasNext();) {
			...
			if (databaseManager.isSimpleElement(elementName) || databaseManager.isMixedElement(elementName)) {
				characterData = databaseManager.getCharacterData(childId.intValue());
			}

			// Create the child Element
			Element childElement = reconstructElement(parent, elementName, attributeNamesToValues, characterData);
	 ...
	 * @param elementAlias - The name of an element in Database nomenclature
	 * @param attributeNamesToValues - String to String, e.g. ["pno"-->"100","colour"-->"red",price-->"20"]
	 */
	private Element reconstructElement(Element parent, String elementAlias, Map attributeNamesToValues, String characterData) {
		Element child = new Element(databaseManager.getOriginalElementName(elementAlias));

		for (Iterator iter = attributeNamesToValues.keySet().iterator(); iter.hasNext();) {
			String attributeAlias = (String) iter.next();
			Object attributeValueObj = (Object) attributeNamesToValues.get(attributeAlias);
			if (attributeValueObj != null) {

				String attributeValue = Nomenclature.convertAttributeValueToString(attributeValueObj);

				String attributeName = databaseManager.getOriginalAttributeName(attributeAlias);
