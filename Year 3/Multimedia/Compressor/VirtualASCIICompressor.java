...
public class VirtualASCIICompressor {
	...
	public static void main(String[] args) {
		...
		encodeSingleChars();
		encodeMostCommonSequences();
		compress();
		table.outputToFile();
		String c = cipherText;
		//System.out.println(cipherText);		
		System.out.println("Size of input: "+ text.length()*8 +" bits");
		System.out.println("Size of output: "+ cipherText.length()+" bits");
		//System.out.println(table.get(0));
		return;
	}
	private static void encodeSingleChars() {
		LookupTable table1 = table;
	...
	private static void encodeMostCommonSequences() {
		//List of Strings
		/*List seqs = new ArrayList();
		//List of Integers
		List ids = new ArrayList();*/

		//Map according to counts
		TreeMap seqs = new TreeMap();

		for (int i = 2; i < 4; i++) {
			for (int j = 0; j < text.length() - i; j++) {
				String seq = text.substring(j, j + i);
				int count = substringOccurences(seq);
				//A preliminary filter to stop the map from exploding
				//if (count > 3) {
				seqs.put(new Integer(count), seq);
				//}
			}
		}
		int mapSize = seqs.size();
		List l = new ArrayList(seqs.values());
		/*for (Iterator iter = seqs.values().iterator(); iter.hasNext();) {
			type element = (type) iter.next();
			
		}*/
		if (mapSize > 256) {
			l = l.subList(mapSize - 256, mapSize - 1);
		}
	...
	private static String formatBinary(String num, int bits) {
		...
		if(num.length() > 8){
			System.out.println("Invalid virtual ASCII char encoded");
			System.exit(-1);
		}
		String acc = "";
