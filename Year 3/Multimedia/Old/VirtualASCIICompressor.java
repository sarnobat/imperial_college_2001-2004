...
public class VirtualASCIICompressor {
	...
	private static void encodeSingleChars() {
		int count = 0;
		for (int i = 0; i < text.length(); i++) {
			table.add(text.substring(i, i + 1));
		}
	}
	...
	private static void compress() {
		...
		while (i < text.length() - 3) {
			String subseq = text.substring(i, i + 3);
			if (table.contains(subseq)) {
				//textBytes.add(new Integer(table.getId(subseq)));
				//System.out.print((byte)table.getId(subseq));
				i += 3;
			}
			else {
				subseq = text.substring(i, i + 2);
				if (table.contains(subseq)) {
					//textBytes.add(new Integer(table.getId(subseq)));
					//System.out.print((byte)table.getId(subseq));
					i += 2;
					...
					subseq = text.substring(i, i + 1);

					int id = table.getId(subseq);
					//System.out.print((byte)table.getId(subseq));
					//textBytes.add(new Integer(id));
					i++;
				}
			}
			int id = table.getId(subseq);
			String binary = Integer.toBinaryString((byte) id);
			//System.out.print(formatBinary(binary,8));
			//System.out.println(id + " (" +(byte) id+")"+ " (" +(char) id+")");
			//System.out.print((char) id);
			//NOTE: CANNOT PRINT CHARACTER VERSIONS OF BYTES BECAUSE THEY COME OUT AS SQUARES
			//WHICH CANNOT BE REPRODUCED PROPERLY
			//Byte b = new Byte((byte) id);
			String formattedBinary = formatBinary(binary,8);
			 
			System.out.print(formattedBinary);
			
			if(id != binaryStringToInt(formattedBinary)){
				System.out.println("Functions not inverted correctly");
			}
		}
	}

	static int binaryStringToInt(String num) {
		if (num.length() != 8) {
			return -1;
		}
		else {
			int acc = 0;
			for (int i = 0; i < 8; i++) {
				acc += Integer.parseInt(num.substring(i, i + 1)) * Math.pow(2, 7 - i);
			}
			return acc;
		}
	}

	private static String formatBinary(String num, int bits) {
		String acc = "";
		int fillerZeros = bits - num.length();
	...
	public static void determineCommonSequences() {

		for (int i = 2; i < 4; i++) {
			for (int j = 0; j < text.length() - i; j++) {
				String seq = text.substring(j, j + i);
				if (table.getId(seq) != -1) {
					continue;
				}
				int count = substringOccurences(seq);
				if (count > 100) {
					table.add(seq);
				}
			}
	...
	private static int substringOccurences(String s) {
		int count = 0;
		for (int i = 0; i < text.length() - s.length(); i++) {
			String currBlock = text.substring(i, i + s.length());
			if (currBlock.equals(s)) {
				count++;
			}
		}
		return count;
